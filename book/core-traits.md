...
It is intentionally minimal and strictly atomic. It includes an associated `Meta` type, allowing implementations to pass state (like a ticket number or guard token) from the acquisition step (`try_lock`) to the release step (`free`).
